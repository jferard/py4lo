...
class TestAddDebugContent(unittest.TestCase):
    ...
    AFTER = b'<table:table-column table:style-name="co1" table:default-cell-style-name="Default"/><table:table-row table:style-name="ro1"><table:table-cell/></table:table-row></table:table><table:named-expressions/></office:spreadsheet></office:body></office:document-content>'

    def test_add_debug_content_empty(self):
        out = io.BytesIO()
        zout = zipfile.ZipFile(out, 'w')
        AddDebugContent({}).call(zout)
        self.assertEquals(TestAddDebugContent.BEFORE+
        b'<office:forms form:automatic-focus="false" form:apply-design-mode="false"></office:forms><table:shapes></table:shapes>'+TestAddDebugContent.AFTER, zout.read("content.xml"))
