...
def get_text_size(oDialogModel: UnoControlModel, text: str) -> Size:
    ...
    """
    oTextControl = cast(UnoTextControl, create_uno_service(Control.FixedText))
...
def message_box(
    ...
    # (thanks Bernard !)
    toolkit = cast(
        UnoToolKit, create_uno_service_ctxt("com.sun.star.awt.Toolkit")
    )
...
class InputBox:
    ...
    def input(
        ...
        dialog = cast(UnoControl, create_uno_service(Control.Dialog))
        ...
        oEditControl.setSelection(
            create_uno_struct(
                "com.sun.star.awt.Selection", Min=0, Max=len(msg_default)
            )
        )
    ...
    def _create_edit_model(self, oDialogModel, name, msg_default):
        oEditModel = oDialogModel.createInstance(ControlModel.Edit)
...
def folder_dialog(title: str, display_dir: StrPath = "") -> Optional[str]:
    ...
    oFolder = cast(
        UnoFolderPicker,
        create_uno_service("com.sun.star.ui.dialogs.FolderPicker"),
    )
    oFolder.Title = title
...
class ConsoleHandler(VoidConsoleHandler):
    def __init__(self, oText: UnoTextControl):
        self._oText = oText
        self.response = None
        self._cur_pos = 0
...
class ConsoleExecutor:
    def __init__(
        self, oDialog: UnoControl, autoclose: bool, oText: UnoTextControl
    ):
        self._oDialog = oDialog
        self._autoclose = autoclose
